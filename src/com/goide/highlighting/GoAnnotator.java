...
public class GoAnnotator implements Annotator {
  ...
  public void annotate(@NotNull PsiElement element, @NotNull AnnotationHolder holder) {
    ...
    if (element instanceof GoContinueStatement) {
      ...
      checkEvaluatedButNotUsed(holder, element, "Function literal evaluated but not used");
    }
    else if (element instanceof GoStringLiteral) {
      checkEvaluatedButNotUsed(holder, element, "String literal evaluated but not used");
    }
  }

  private static void checkEvaluatedButNotUsed(AnnotationHolder holder, PsiElement element, String message) {
    ...
    if (element instanceof GoFunctionLit) {
      ...
      if (parent instanceof GoIndexOrSliceExpr ||
          parent instanceof GoConditionalExpr) return;
    }
    ...
    if (parent instanceof GoLeftHandExprList &&
        ((GoLeftHandExprList)parent).getExpressionList().size() != 1) return;
