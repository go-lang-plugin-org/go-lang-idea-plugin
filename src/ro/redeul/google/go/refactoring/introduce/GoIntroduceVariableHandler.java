...
public class GoIntroduceVariableHandler extends GoIntroduceHandlerBase {
    ...
    private boolean introduceExpressionStatement(Editor editor, PsiElement element, String declaration)
        ...
        if (resultNames.isEmpty()) {
            throw new GoRefactoringException(GoBundle.message("error.expression.has.void.return.type"));
        }
    ...
    private GoFunctionDeclaration resolveToFunctionDeclaration(PsiElement element) {
        ...
        GoLiteralIdentifier id = getFunctionIdentifier((GoCallOrConvExpression) element);
        if (id == null) {
            return null;
        }
        ...
        if (resolve instanceof GoFunctionDeclaration) {
            return (GoFunctionDeclaration) resolve;
        }
        return null;
    }
