...
class GoBinaryExpressionBlock extends GoExpressionBlock<GoBinaryExpression<?>> {
    ...
    GoBinaryExpressionBlock(@NotNull GoBinaryExpression node, CommonCodeStyleSettings settings,
                            Indent indent) {
    ...
    protected void setCutoff(int cutoff) {
        this.cutoff = cutoff;
        if (getPsi().getOp().precedence() >= cutoff)
    ...
    protected Block customizeBlock(@NotNull Block childBlock, @NotNull PsiElement childPsi) {
        if (childBlock instanceof GoBinaryExpressionBlock) {
            ((GoBinaryExpressionBlock)childBlock).setCutoff(cutoff);
        }
    ...
    private BinaryExpressionInfo walkBinary(GoBinaryExpression expr) {
        ...
        */
        if (expr.getLeftOperand() instanceof GoBinaryExpression) {
            ...
            if (!(left.getOp().precedence() < exprOp.precedence())) {
...
*/
        if (expr.getRightOperand() instanceof GoBinaryExpression) {
            ...
            if (!(right.getOp().precedence() <= exprOp.precedence())) {
