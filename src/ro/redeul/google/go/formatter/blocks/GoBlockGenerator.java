...
public class GoBlockGenerator {
  ...
  public static Block generateBlock(ASTNode node, Indent indent, Alignment alignment, CommonCodeStyleSettings styleSettings) {
    ...
    if (psi instanceof GoFile)
      return new GoFileBlock((GoFile) psi, styleSettings);

    if (psi instanceof GoPackageDeclaration)
      return new GoSyntheticBlock<GoPackageDeclaration>((GoPackageDeclaration) psi, styleSettings);

    if (psi instanceof GoImportDeclarations)
      return new GoImportsBlock((GoImportDeclarations) psi, styleSettings);

    if (psi instanceof GoImportDeclaration)
      return new GoSyntheticBlock<GoImportDeclaration>(
        (GoImportDeclaration) psi,
        styleSettings,
        indent);
